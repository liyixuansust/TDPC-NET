...
def main():
    ...
    num_iters = args.num_iters or (len(train_set) * args.num_epochs) // args.batch_size  # 20937
    ...
    enum_batches = len(train_set)/float(args.batch_size)  # nums_batch per epoch 335/8->42

    losses = AverageMeter()
    torch.set_grad_enabled(True)
    sum = 0
    viz = Visdom(port=6666)
    viz.line([1.], [0.], win="train_loss_epo",opts=dict(title='train loss epo', xlabel='epoch', ylabel='train_loss_epo'))

    for i, data in enumerate(train_loader, args.start_iter):  #

        elapsed_bsize = int(i / enum_batches)+1  #
        epoch = int((i + 1) / enum_batches)
        s = (epoch + 1) // args.batch_size
        ...
        msg = 'Iter {0:}, Epoch {1:}, Loss {2:.7f}'.format(i+1, int((i+1)/enum_batches), losses.avg)
        logging.info(msg)
