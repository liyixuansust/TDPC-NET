...
__all__ = ['sigmoid_dice_loss', 'softmax_dice_loss', 'GeneralizedDiceLoss', 'FocalLoss', 'FocalTverskyDiceLoss', 'mixingdiceLoss']

cross_entropy = F.cross_entropy


def FocalLoss(output, target, gamma=4):
...
def MixedLoss(output, target, eps=1e-5, weight_type='square', alpha=0.3,gamma=4):
    # 将标签中的4替换为3
    target[target == 4] = 3
    # 将目标张量展平为二维张量
    target = expand_target(target, n_class=output.size()[1]) 
    output1 = flatten(output)[1:, ...]  
    target1 = flatten(target)[1:, ...]
    # 计算目标中每个类别的像素数量
    target_sum = target1.sum(-1)
    if weight_type == 'square':
        class_weights = 1. / (target_sum * target_sum + eps)
    elif weight_type == 'identity':
        class_weights = 1. / (target_sum + eps)
    ...
    # 计算交集和并集
    intersect = (output1 * target1).sum(-1)
    intersect_sum = (intersect * class_weights).sum()
    denominator = (output1 + target1).sum(-1)
    denominator_sum = (denominator * class_weights).sum() + eps
    ...
    gd_loss = 1 - 2. * (intersect * class_weights).sum() / ((denominator * class_weights).sum() + eps)
    # 计算Focal Loss
    output = output.view(output.size(0), output.size(1), -1)  # N,C,H,W,D => N,C,H*W*D
    output = output.transpose(1, 2)  # N,C,H*W*D => N,H*W*D,C
    output = output.contiguous().view(-1, output.size(2))  # N,H*W*D,C => N*H*W*D,C
    ...
    # 计算加权混合损失
    weighted_loss = (1 - alpha) * gd_loss + alpha * focal_loss

    return weighted_loss

def FocalTverskyDiceLoss(output, target, eps=1e-5, alpha=0.4, gamma=1.0):
    ...
    false_neg = ((1-output) * target).sum(-1)
    false_pos = (output * (1-target)).sum(-1)
    denominator = (output + target).sum(-1)
    denominator_sum = (intersect + alpha * false_neg + (1 - alpha) * false_pos).sum()
    ...
    y = 1 - (intersect_sum + 1) / (denominator_sum + 1)

    return pow((1-y), gamma)
...
def mixingdiceLoss(output, target, eps=1e-5, weight_type='square', epoch=None):  # Generalized dice loss
    ...
    loss1 = (1 - epoch / 900) * (intersect0[0] / (denominator0[0] + eps)) + 2 * epoch / 900 * (intersect1[0] / (denominator1[0] + eps))
    loss2 = (1 - epoch / 900) * (intersect0[1] / (denominator0[1] + eps)) + 2 * epoch / 900 * (intersect1[1] / (denominator1[1] + eps))
    loss3 = (1 - epoch / 900) * (intersect0[2] / (denominator0[2] + eps)) + 2 * epoch / 900 * (intersect1[2] / (denominator1[2] + eps))
    logging.info('1:{:.4f} | 2:{:.4f} | 4:{:.4f}'.format(loss1.data, loss2.data, loss3.data))
    #print(1)
    return (1 - epoch / 900) * (1 - intersect_sum0 / denominator_sum0) + 2 * epoch / 900 * (1 - intersect_sum1 / denominator_sum1)
...
def jaccardLoss(output, target, eps=1e-5, weight_type='square', epoch=None):  # Generalized dice loss
    ...
    loss1 = (1 - epoch / 900) * (intersect0[0] / (denominator0[0] + eps)) + 2 * epoch / 900 * (
                intersect1[0] / (denominator1[0] + eps))
    loss2 = (1 - epoch / 900) * (intersect0[1] / (denominator0[1] + eps)) + 2 * epoch / 900 * (
                intersect1[1] / (denominator1[1] + eps))
    loss3 = (1 - epoch / 900) * (intersect0[2] / (denominator0[2] + eps)) + 2 * epoch / 900 * (
                intersect1[2] / (denominator1[2] + eps))
    logging.info('1:{:.4f} | 2:{:.4f} | 4:{:.4f}'.format(loss1.data, loss2.data, loss3.data))
    print(1)
    return (1 - epoch / 900) * (1 - intersect_sum0 / denominator_sum0) + 2 * epoch / 900 * (
